...
CREATE TABLE pro_board (
     b_num NUMBER GENERATED ALWAYS AS IDENTITY (NOCACHE) PRIMARY KEY,        -- 게시글 번호 (자동 증가)
 ...
 CREATE TABLE pro_team (
     t_id NUMBER GENERATED ALWAYS AS IDENTITY (NOCACHE) PRIMARY KEY, -- 팀 번호 (자동 증가)
...
CREATE TABLE todo_list(
    td_id NUMBER GENERATED ALWAYS AS IDENTITY (NOCACHE) PRIMARY KEY,  -- 할 일 고유 ID (자동 증가) == calendarNo
    ...
    td_todo VARCHAR2(255) NOT NULL,      -- 일정
